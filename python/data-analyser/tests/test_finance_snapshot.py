...
from data_analyser.snapshot import normalize_entries, spreadsheet_xml, snapshot_json, number, MANAGED_SHEETS
...
def test_desktop_json_keeps_text_and_rejects_unmanaged_sheets():
    snapshot = {
        "schema": "2", "company": "company_1", "month": "2026-08",
        "refreshed_at": "2026-08-31T00:00:00Z",
        "sheets": {"凭证明细": [["123456789012345678", "=1+1", 12.3, None]]},
    }
    result = json.loads(snapshot_json(snapshot))
    assert result["sheets"]["凭证明细"][0] == ["123456789012345678", "=1+1", 12.3, None]
    snapshot["sheets"] = {"预算输入": [[1]]}
    with pytest.raises(CheckFailure):
        snapshot_json(snapshot)
    snapshot["sheets"] = {"利润表": [[float("nan")]]}
...
def test_snapshot_removes_auxiliary_totals_and_adds_nonauxiliary_accounts(tmp_path, selected_month):
    from data_analyser.snapshot import collect_snapshot
    ...
    (tmp_path / "runtime/registry/accountbooks.json").write_text(json.dumps({"version": 2, "accountbooks": [
        {"key": "company_1", "company_id": "1", "name": "测试公司", "enabled": True, "login_account": "account_1", "session_file": "http_sessions/unused.json"}
    ]}))
    class Client:
