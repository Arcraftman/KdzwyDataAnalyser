...
from data_analyser.build_template import build_workbook, save_template
...
def test_refuses_existing_output_and_failed_build_preserves_it(tmp_path):
    ...
    with patch('data_analyser.build_template.build_workbook', side_effect=RuntimeError('failed')):
...
def test_cli_builds_from_empty_workspace_without_node_or_existing_template(tmp_path):
    ...
    (tmp_path / 'config/finance_read_sources.json').write_bytes((ROOT / 'config/finance_read_sources.json').read_bytes())
    env = {**os.environ, 'DATA_ANALYSER_ROOT': str(tmp_path), 'PYTHONPATH': str(ROOT / 'python'), 'PYTHONUTF8': '1'}
    command = [sys.executable, '-m', 'data_analyser.build_template']
