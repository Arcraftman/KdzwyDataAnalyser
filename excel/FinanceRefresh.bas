...
Public Sub LoginFinance()
    ...
    scriptPath = rootPath & "\scripts\start-local.ps1"
...
Public Sub ConfigureDeepSeek()
    ...
    scriptPath = WorkbookProjectRoot() & "\scripts\configure-deepseek.ps1"
    If Len(Dir$(scriptPath)) = 0 Then Err.Raise 513, , "DeepSeek 配置脚本不存在"
    CreateObject("WScript.Shell").Run "powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File " & QuoteArgument(scriptPath), 1, False
End Sub
